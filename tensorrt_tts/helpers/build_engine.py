...
def build_engine(
    ...
    max_output_len: int = 8192,  # ~60+ seconds of audio (7 tokens per frame, ~85ms per frame)
    ...
    # max_seq_len = 512 + 8192 = 8704 tokens (~100s of audio)
    # 7 tokens = 1 frame = ~85ms, so 8192 tokens = ~99 seconds
    build_config = BuildConfig(
        max_input_len=max_input_len,
        max_seq_len=max_input_len + max_output_len,
        max_batch_size=max_batch_size,
        max_num_tokens=max_input_len + max_output_len,  # Match max_seq_len
    )
...
def main(
    quantization: str = "fp16",
    max_input_len: int = 512,
    max_output_len: int = 8192,  # ~100 seconds of audio
    verify: bool = True,
):
